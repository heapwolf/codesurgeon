        ...
        case 'stat':
          ...
          if(level === 'toplevel') {
            ...
            for (i = 0, sl = args.length; i < sl; i++) {
              ...
              if (name === args[i]) {
                if (chunks.length > 2) {
                  that.output += chunks[0] + '=typeof ' + chunks[0] +
                                 '==="undefined"?{}:' + chunks[0] + '\n';

                  chunks.slice(1).reduce(function (acc, name) {
                    var stat = acc + '.' + name;
                    that.output += stat + '||(' + stat + '={});\n';
                    return stat;
                  });
                }
                that.output += uglify.uglify.gen_code(this.parent.node, true);
...
Codesurgeon.prototype.append = function (file) {
  !this.options.quiet && console.log('Attempting to append to file [' + file.green + ']');
