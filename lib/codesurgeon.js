...
Codesurgeon.prototype.extract = function (identifiers) {
        ...
        if (level === 'toplevel') {
          for (i = 0, sl = args.length; i < sl; i++) {
            ...
            if (this.parent.node[1][0][0] === args[i]) {
              output[i] = uglify.uglify.gen_code(this.parent.node, { indent_level: 4, beautify: true });
            }
          }
        }
      break;
  ...
  this.output += output.join(that.options.seperator)
  return this;
};
...
Codesurgeon.prototype.uglify = function (options) {
  !this.options.quiet && console.log('Attempting to uglify code.');
